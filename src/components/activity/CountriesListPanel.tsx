...
import { countryCatalog, selectCurrentContinent, selectCurrentCountryCode } from "src/store/CountryStore/slice";
import type { CountryData } from "src/store/CountryStore/types";
import { useAppSelector } from "src/store/hooks";
import { cn } from "src/utils/styles";

function countryCodesToCountryData(countryCodes: string[]) {
  const countryData: CountryData[] = [];
  for (const countryCode of countryCodes) {
    const data = countryCatalog[countryCode];
    if (data) countryData.push(data);
  }
  return countryData;
}
...
export default function CountriesListPanel({ isAbridged = false }: { isAbridged?: boolean }) {
  ...
  const getCurrentCountryA3 = selectCurrentCountryCode(activity?.activity);
  ...
  const visited = useMemo(() => countryCodesToCountryData(visitedCountries), [visitedCountries]);
  const unvisited = useMemo(() => countryCodesToCountryData(unvisitedCountries), [unvisitedCountries]);
